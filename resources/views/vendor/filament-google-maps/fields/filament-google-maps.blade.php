<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    ...
    <script src="{{ asset('js/filament-google-maps.js') }}"></script>

    <div x-data="filamentGoogleMapsField({
            ...
            if (value?.lat && value?.lng) {
                $wire.set('data.latitude', value.lat);
                $wire.set('data.longitude', value.lng);
            }
        })" id="{{ $getId() . '-alpine' }}" wire:ignore>
        @if ($isSearchBoxControlEnabled())
            <input x-ref="pacinput" type="text" placeholder="Search Box" />
        @endif

        <div x-ref="map" class="w-full" style="
                height: {{ $getHeight() }};
                min-height: 30vh;
                z-index: 1 !important;
            "></div>
